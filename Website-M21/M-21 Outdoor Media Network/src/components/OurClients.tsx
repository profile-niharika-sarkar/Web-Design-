...
export default function OurClients() {
  ...
  return (
    <section id="clients" ref={ref} className="pt-24 pb-12 overflow-hidden relative bg-transparent border-y border-zinc-900/40">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-brand-purple/5 rounded-full blur-[120px] pointer-events-none animate-pulse-slow" />
      <div className="absolute top-1/2 left-1/3 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] bg-brand-pink/2 rounded-full blur-[100px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6 mb-16 relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-pink-500/5 border border-pink-500/10 rounded-full text-[14px] text-brand-pink font-medium mb-8">
            <span className="w-2 h-2 rounded-full bg-brand-pink animate-pulse" />
            Trusted Partners
          </div>
          
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-black leading-[1.05] tracking-[-0.03em] mb-10 text-white select-none whitespace-normal">
            Collaborations Across <span className="bg-gradient-to-r from-brand-pink via-brand-magenta to-brand-purple bg-clip-text text-transparent inline-block">Sectors</span>
          </h2>
        ...
        <div className="relative overflow-hidden mb-1">
          <div className="flex gap-6 w-[200%] animate-[marquee_8s_linear_infinite] hover:[animation-play-state:paused] cursor-pointer">
            {repeatedRow1.map((logo, i) => (
              <div
                key={i}
                className="flex items-center justify-center w-[250px] h-[100px] rounded-xl bg-zinc-950/20 backdrop-blur-md border border-zinc-900/60 group hover:border-brand-pink/30 hover:bg-zinc-900/10 transition-all duration-300 px-6 shrink-0 shadow-[inset_0_1px_1px_rgba(255,255,255,0.01)]"
              >
                <img
                  src={`/Logos/${logo}`}
                  alt={logo}
